# 김홍래 | Backend Developer

- 목표: 안정성과 확장성을 갖춘 백엔드 시스템 구축
- 강점: 문서화 기반 협업, 테스트/리팩토링 습관, 배포/운영까지 연결

## 📁 Projects

### MatchMyDuo - 게임 플레이어들이 쉽고 빠르게 듀오/파티를 찾을 수 있는 플랫 
> 개발 기간: 2025.12.10 ~ 2026.01.07 | 인원: 6명 (BE 6) <br>
> 역할: 실시간 채팅, Redis 도입 <br>
> 기술 스택: <br>
> 성과: <br>
> Repo: [MatchMyDuo](https://github.com/prgrms-web-devcourse-final-project/WEB7_9_FinalScreening_BE)

### UniMate - 대학생 생활 기반 룸메이트 매칭 서비스 
> 개발 기간: 2025.10.10 ~ 2025.11.20 | 인원: 5명 (BE 5) <br>
> 역할: 자동/사용자 선택 필터, 응답 처리/상태 결정, 최종 확정/거절, 리뷰 구현 <br>
> 기술 스택: <br>
> 성과: <br>
> Repo: [Unimate](https://github.com/prgrms-be-devcourse/NBE7-9-3-Team10)

### chuncheon-guide-chatbot - 춘천 여행 가이드 챗봇 
> 개발 기간: <br>
> 역할: Ollama 모델 튜닝 <br>
> 기술 스택: <br>
> 성과 <br>
> Repo: [chuncheon-guide-chatbot](https://github.com/HongRae-Kim/chuncheon-guide-chatbot)

### hallym_2GiJang - APG 데이터 분석 및 AI 모델을 통한 솔루션 제공 서비스 
> 개발 기간: <br>
> 역할: <br>
> 기술 스택: <br>
> 성과: <br>
> Repo: [hallym_2GiJang](https://github.com/ysJang0104/hallym_2GiJang)

---

## 🎓 Education
- 프로그래머스 데브코스 클라우드 기반 백엔드 엔지니어링 (2025.07 ~ 2026.01)
  -   
